from __future__ import annotations

import json
import tempfile
import unittest
from dataclasses import asdict
from pathlib import Path
from unittest import mock

from ourd.errors import PolicyError, ProviderError
from ourd.providers.base import ProviderConfig
from ourd.reasoning.benchmark import (
    BENCHMARK_SYSTEM_IDS,
    BenchmarkTask,
    SourceFileRecord,
    merge_benchmark_runs,
    run_benchmark,
)
from ourd.reasoning.ablation import standard_ablation_configurations
from ourd.reasoning.model_benchmark import (
    MODEL_EXECUTION_MODE,
    MODEL_HELD_OUT_QUALIFICATION_STATUS,
    MODEL_QUALIFICATION_STATUS,
    BaseModelBenchmarkExecutor,
    InstrumentedProvider,
    OIECBenchmarkExecutor,
    RuntimeEnvironment,
    _canonical_benchmark_answer,
    bind_provider_profile,
    close_model_benchmark_executors,
    make_model_benchmark_executors,
    release_ollama_runtime,
)
from ourd.reasoning.models import stable_hash
from ourd.reasoning.qualification import qualify_reasoning_runs
from ourd.reasoning.generator import response_text
from ourd.reasoning.verifier import PROCESS_CHECKS
from tools.run_reasoning_model_benchmark import DEFAULT_SOURCE_PATHS, ROOT, _write_new


def runtime_environment(allocation: str = "100% GPU") -> RuntimeEnvironment:
    return RuntimeEnvironment(
        schema_version=1,
        platform_system="Linux",
        kernel_release="test-kernel",
        architecture="x86_64",
        python_version="3.12.0",
        cpu_model="Test CPU",
        logical_cpu_count=8,
        memory_bytes=16_000_000_000,
        gpu_name="Test GPU",
        gpu_uuid="GPU-test",
        gpu_driver="1.0",
        gpu_memory_bytes=16_000_000_000,
        accelerator_allocation=allocation,
        runtime_context_tokens=8192,
        clock_source="time.perf_counter_ns",
    )


def task() -> BenchmarkTask:
    return BenchmarkTask.from_dict(
        {
            "schema_version": 1,
            "problem_id": "logic-test",
            "category": "logic",
            "prompt": "All governed mutations require EON. No EON action exists. Is mutation permitted?",
            "oracle": {"kind": "exact", "expected": "no"},
            "oracle_method": "modus tollens",
            "required_evidence_ids": ["logic:eon-required"],
            "required_counterexamples": [],
            "source_refs": ["README.md"],
        }
    )


def provider_bound_run(
    *,
    ablation_id: str = "full_sr",
    qualification_status: str = MODEL_HELD_OUT_QUALIFICATION_STATUS,
    model_digest: str = "a" * 64,
):
    source_files = (SourceFileRecord(path="fixture", sha256="c" * 64),)
    source_hash = stable_hash([asdict(item) for item in source_files])
    ablation = next(
        item
        for item in standard_ablation_configurations()
        if item.ablation_id == ablation_id
    )
    executors = make_model_benchmark_executors(
        provider_factory=lambda: FakeBoundProvider(model_digest=model_digest),
        source_snapshot_hash=source_hash,
        runtime_collector=lambda _: runtime_environment(),
        runtime_releaser=lambda _: None,
        ablation=ablation,
    )
    try:
        return run_benchmark(
            tasks=(task(),),
            executors=executors,
            generated_on="2026-08-29",
            package_version="test",
            git_head="head",
            worktree_dirty=True,
            source_files=source_files,
            execution_mode=MODEL_EXECUTION_MODE,
            qualification_status=qualification_status,
        )
    finally:
        close_model_benchmark_executors(executors)


class FakeBoundProvider:
    def __init__(self, *, model_digest: str = "a" * 64, fail: bool = False):
        self.config = ProviderConfig(
            model="fake-bound-model",
            base_url="http://127.0.0.1:11434/v1",
            reasoning_effort="medium",
            json_object_output=True,
            max_output_tokens=2048,
            context_budget_tokens=6000,
            timeout_seconds=60,
            max_transport_retries=0,
            max_reasoning_samples=16,
        )
        self.model_digest = model_digest
        self.fail = fail
        self.requests = []

    def preflight(self):
        return {
            "provider": "legacy_http_fixture",
            "model": self.config.model,
            "base_url": self.config.base_url,
            "endpoint_type": "ollama_local",
            "context_budget_tokens": self.config.context_budget_tokens,
            "max_output_tokens": self.config.max_output_tokens,
            "reasoning_effort": self.config.reasoning_effort,
            "structured_output_mode": "json_object",
            "supports_json_object": True,
            "ollama_version": "0.32.14",
            "sampling_contract": {
                "temperature_bp": 0,
                "top_p_bp": 10_000,
                "seed": 1234,
            },
            "max_transport_retries": 0,
            "max_reasoning_samples": 16,
            "status": "ready",
            "model_family": "fake",
            "parameter_size": "1B",
            "quantization_level": "Q4",
            "model_info": {
                "general.sampling.temp": 1,
                "general.sampling.top_p": 0.95,
            },
            "model_digest": self.model_digest,
            "model_size": 1_000_000,
        }
    def _response(self, text: str):
        return {
            "model": self.config.model,
            "status": "completed",
            "temperature": 1,
            "top_p": 1,
            "usage": {"input_tokens": 10, "output_tokens": 5, "total_tokens": 15},
            "output": [
                {
                    "type": "reasoning",
                    "encrypted_content": "private-reasoning-must-not-persist",
                },
                {
                    "type": "message",
                    "content": [{"type": "output_text", "text": text}],
                },
            ],
        }

    def create_response(self, *, instructions, input_items, tools):
        self.requests.append(
            {"instructions": instructions, "input_items": input_items, "tools": tools}
        )
        if self.fail:
            raise ProviderError("synthetic provider failure")
        lowered = instructions.casefold()
        payload = json.loads(input_items[0]["content"])
        if "proposer" in lowered:
            problem = payload["problem"]
            hypothesis_ids = [item["hypothesis_id"] for item in payload["hypotheses"]]
            perspective = payload["perspective"]
            inference_by_perspective = {
                "direct": "deductive",
                "mechanistic": "causal",
                "counterexample_first": "defeasible",
                "assumption_inversion": "abductive",
            }
            return self._response(
                json.dumps(
                    {
                        "conclusion": "no",
                        "hypothesis_ids": hypothesis_ids[:1],
                        "provider_confidence_bp": 9500,
                        "estimated_cost_bp": 500,
                        "goal_relevance_bp": 9000,
                        "risk_bp": 500,
                        "steps": [
                            {
                                "step_id": f"{perspective}-step",
                                "claim": f"{perspective} supports no",
                                "premises": ["problem", hypothesis_ids[0]],
                                "evidence_ids": problem["evidence_ids"],
                                "inference": inference_by_perspective[perspective],
                                "confidence_bp": 9500,
                                "assumptions": [],
                                "falsifier": "An EON action exists.",
                            }
                        ],
                    }
                )
            )
        if "process verifier" in lowered:
            candidate = payload["candidate"]
            return self._response(
                json.dumps(
                    {
                        "steps": [
                            {
                                "step_id": item["step_id"],
                                "checks": {name: True for name in PROCESS_CHECKS},
                                "failures": [],
                            }
                            for item in candidate["steps"]
                        ],
                        "contradictions": [],
                    }
                )
            )
        if "falsifier" in lowered:
            return self._response(
                json.dumps(
                    {
                        "searched_falsifiers": ["An EON action exists."],
                        "counterexamples": [],
                        "contradicted_step_ids": [],
                        "unresolved_defeat_conditions": [],
                        "critical": False,
                        "survival_bp": 9000,
                    }
                )
            )
        if "synthesizer" in lowered:
            return self._response(
                json.dumps(
                    {
                        "conclusion": "no",
                        "source_path_ids": [payload["selected_winner"]],
                    }
                )
            )
        return self._response(
            json.dumps(
                {
                    "answer": "no",
                    "confidence_bp": 9000,
                    "evidence_ids": ["logic:eon-required"],
                    "counterexamples": [],
                    "terminal_state": "ANSWER",
                }
            )
        )


class FakeDirectBoundProvider(FakeBoundProvider):
    def __init__(self):
        super().__init__()
        self.config.provider_kind = "llama_cpp_process"
        self.config.base_url = ""
        self.last_completion_request_sent = False
        self.close_calls = 0

    def create_response(self, *, instructions, input_items, tools):
        self.last_completion_request_sent = True
        return super().create_response(
            instructions=instructions,
            input_items=input_items,
            tools=tools,
        )

    def close(self):
        self.close_calls += 1

    def preflight(self):
        return {
            "status": "ready",
            "provider": "llama_cpp_process",
            "endpoint_type": "llama_cpp_process",
            "model": self.config.model,
            "model_digest": "b" * 64,
            "model_file_size": 12_000_000_000,
            "model_architecture": "qwen35",
            "parameter_count": "27.32B",
            "quantization": "Q2_K",
            "runner_identity": {"sha256": "c" * 64},
            "sampling_contract": {
                "seed": 1234,
                "temperature_bp": 1000,
                "top_p_bp": 9500,
                "top_k": 40,
            },
        }


class ModelBenchmarkTests(unittest.TestCase):
    def test_exact_boolean_answers_are_canonicalized_without_oracle_leakage(self) -> None:
        boolean_task = task()
        self.assertEqual("no", _canonical_benchmark_answer(boolean_task, "No, mutation is blocked."))
        self.assertEqual(
            "no",
            _canonical_benchmark_answer(
                boolean_task,
                "Not proven: the release lacks current evidence.",
            ),
        )
        yes_task = BenchmarkTask.from_dict(
            {
                "schema_version": 1,
                "problem_id": "logic-yes-test",
                "category": "logic",
                "prompt": "Is the exact-hash approval present?",
                "oracle": {"kind": "exact", "expected": "yes"},
                "oracle_method": "exact",
                "required_evidence_ids": [],
                "required_counterexamples": [],
                "source_refs": [],
            }
        )
        self.assertEqual("yes", _canonical_benchmark_answer(yes_task, "Yes: approval exists."))
        self.assertEqual(
            "Ambiguous explanatory prose",
            _canonical_benchmark_answer(yes_task, "Ambiguous explanatory prose"),
        )

    def test_benchmark_source_surface_covers_reasoning_and_direct_runner(self) -> None:
        source_paths = set(DEFAULT_SOURCE_PATHS)
        expected = {
            path.relative_to(ROOT).as_posix()
            for directory in (
                ROOT / "ourd/reasoning",
                ROOT / "ourd/providers",
                ROOT / "grammars/providers",
                ROOT / "native/oiec_llama_runner",
                ROOT / "schemas/providers",
            )
            for path in directory.rglob("*")
            if path.is_file() and "__pycache__" not in path.parts
        }
        self.assertTrue(expected <= source_paths)
        self.assertIn("README.md", source_paths)
        self.assertIn("benchmarks/reasoning/tasks/qualification-v1.jsonl", source_paths)
        self.assertIn("benchmarks/reasoning/tasks/qualification-v2.jsonl", source_paths)
        self.assertIn("tools/qualify_reasoning_runs.py", source_paths)
        self.assertFalse(any(path.startswith("benchmarks/reasoning/runs/") for path in source_paths))

    def test_direct_benchmark_shares_one_fresh_context_runner(self) -> None:
        providers = []

        def provider_factory():
            provider = FakeDirectBoundProvider()
            providers.append(provider)
            return provider

        executors = make_model_benchmark_executors(
            provider_factory=provider_factory,
            source_snapshot_hash="b" * 64,
            runtime_collector=lambda _: runtime_environment(),
            runtime_releaser=lambda _: None,
        )

        self.assertEqual(1, len(providers))
        self.assertTrue(
            all(executor.provider.provider is providers[0] for executor in executors)
        )

    def test_shared_direct_provider_is_closed_once_after_benchmark(self) -> None:
        provider = FakeDirectBoundProvider()
        executors = make_model_benchmark_executors(
            provider_factory=lambda: provider,
            source_snapshot_hash="b" * 64,
            runtime_collector=lambda _: runtime_environment(),
            runtime_releaser=lambda _: None,
        )

        close_model_benchmark_executors(executors)

        self.assertEqual(1, provider.close_calls)

    def test_provider_binding_requires_exact_model_digest(self) -> None:
        with self.assertRaises(PolicyError):
            bind_provider_profile(FakeBoundProvider(model_digest=""))

    def test_provider_binding_includes_structured_output_and_runtime_version(self) -> None:
        profile = bind_provider_profile(FakeBoundProvider())
        self.assertIn("structured_output=json_object", profile.decoding)
        self.assertIn("ollama_version=0.32.14", profile.decoding)
        self.assertIn("temperature_bp=0", profile.decoding)
        self.assertIn("top_p_bp=10000", profile.decoding)
        self.assertIn("seed=1234", profile.decoding)

    def test_direct_provider_binding_uses_process_identity(self) -> None:
        profile = bind_provider_profile(FakeDirectBoundProvider())
        self.assertEqual("llama_cpp_process", profile.endpoint_type)
        self.assertEqual(f"process://{'c' * 64}", profile.base_url)
        self.assertEqual("qwen35", profile.model_family)
        self.assertEqual("Q2_K", profile.quantization_level)
        self.assertEqual("1234", profile.seed)

    def test_provider_binding_rejects_credentials_in_base_url(self) -> None:
        provider = FakeBoundProvider()
        provider.config.base_url = "http://user:secret@127.0.0.1:11434/v1"
        with self.assertRaises(PolicyError):
            bind_provider_profile(provider)

    def test_direct_executor_does_not_persist_private_reasoning(self) -> None:
        provider = FakeBoundProvider()
        executor = BaseModelBenchmarkExecutor(
            system_id="base",
            provider=provider,
            source_snapshot_hash="b" * 64,
            runtime_collector=lambda _: runtime_environment(),
        )
        observation = executor.execute(task())
        descriptor = executor.descriptor()
        serialized = json.dumps({"observation": asdict(observation), "descriptor": descriptor})
        self.assertEqual("no", observation.answer)
        self.assertEqual(15, observation.token_count)
        self.assertNotIn("private-reasoning-must-not-persist", serialized)
        self.assertEqual(1, descriptor["telemetry"]["provider_calls"])
        self.assertEqual(1, len(descriptor["telemetry"]["response_hashes"]))
        self.assertEqual(1, len(descriptor["telemetry"]["runtime_observation_hashes"]))

    def test_oracle_answer_and_method_do_not_enter_direct_provider_context(self) -> None:
        provider = FakeBoundProvider()
        executor = OIECBenchmarkExecutor(
            system_id="oiec",
            provider=provider,
            source_snapshot_hash="b" * 64,
            runtime_collector=lambda _: runtime_environment(),
        )
        executor.execute(task())
        request = json.dumps(provider.requests[0], sort_keys=True)
        self.assertNotIn("modus tollens", request)
        self.assertNotIn('"expected"', request)
        self.assertIn("for yes/no prompts exactly yes or no", request)

    def test_provider_failure_is_recorded_as_result(self) -> None:
        executor = BaseModelBenchmarkExecutor(
            system_id="base",
            provider=FakeBoundProvider(fail=True),
            source_snapshot_hash="b" * 64,
            runtime_collector=lambda _: runtime_environment(),
        )
        observation = executor.execute(task())
        self.assertEqual("INSUFFICIENT_EVIDENCE", observation.terminal_state)
        self.assertEqual(1, observation.collisions)
        self.assertEqual(0, observation.retries)

    def test_non_answer_terminal_with_answer_fails_closed(self) -> None:
        provider = FakeBoundProvider()

        def inconsistent_response(**_):
            return provider._response(
                json.dumps(
                    {
                        "answer": "no",
                        "confidence_bp": 3000,
                        "evidence_ids": ["logic:eon-required"],
                        "counterexamples": [],
                        "terminal_state": "INSUFFICIENT_EVIDENCE",
                    }
                )
            )

        provider.create_response = inconsistent_response
        executor = OIECBenchmarkExecutor(
            system_id="oiec",
            provider=provider,
            source_snapshot_hash="b" * 64,
            runtime_collector=lambda _: runtime_environment(),
        )
        observation = executor.execute(task())
        self.assertIn("non-answer terminal state", observation.answer)
        self.assertEqual("INSUFFICIENT_EVIDENCE", observation.terminal_state)
        self.assertEqual(1, observation.collisions)

    def test_direct_in_call_failure_records_runtime_without_response_telemetry(self) -> None:
        provider = FakeDirectBoundProvider()
        provider.fail = True
        instrumented = InstrumentedProvider(
            provider,
            runtime_probe=lambda _: runtime_environment(),
        )

        with self.assertRaisesRegex(ProviderError, "synthetic provider failure"):
            instrumented.create_response(
                instructions="Answer",
                input_items=[{"role": "user", "content": "hello"}],
                tools=[],
            )

        self.assertEqual(1, instrumented.calls)
        self.assertEqual(1, instrumented.failures)
        self.assertEqual(1, instrumented.runtime_observation_count)
        self.assertEqual(1, len(instrumented.runtime_observation_hashes))
        self.assertEqual([], instrumented.response_hashes)
        self.assertEqual(0, instrumented.total_tokens)
        self.assertEqual(1, len(instrumented.failure_records))
        self.assertEqual("provider_call", instrumented.failure_records[0]["phase"])
        self.assertEqual(
            "synthetic provider failure",
            instrumented.failure_records[0]["error_message"],
        )

        instrumented.record_reasoning_repair(
            role="verifier_batch",
            reason="synthetic cardinality failure",
            item_ids=("p1", "p2"),
        )
        snapshot = instrumented.snapshot()
        self.assertEqual(1, snapshot.validation_repairs)
        self.assertEqual("verifier_batch", instrumented.repair_records[0]["role"])

    def test_reasoning_batch_halts_after_first_malformed_response(self) -> None:
        provider = FakeBoundProvider()
        provider.create_response = lambda **_: provider._response("")
        instrumented = InstrumentedProvider(provider)
        responses = instrumented.create_responses(
            requests=[
                {"instructions": "proposer", "input_items": [], "tools": []}
                for _ in range(4)
            ],
            max_responses=4,
        )
        self.assertEqual(4, len(responses))
        self.assertEqual(1, instrumented.calls)
        self.assertEqual(1, instrumented.failures)
        self.assertEqual(
            "structured_response_validation",
            instrumented.failure_records[0]["phase"],
        )
        self.assertTrue(all(item["type"] == "reasoning_error" for item in responses))

    def test_verifier_batch_preserves_malformed_payload_for_bounded_repair(self) -> None:
        provider = FakeBoundProvider()
        provider.create_response = lambda **_: provider._response('{"steps":[')
        instrumented = InstrumentedProvider(provider)
        responses = instrumented.create_responses(
            requests=[
                {
                    "instructions": "process verifier",
                    "input_items": [],
                    "tools": [],
                    "allow_invalid_json": True,
                }
                for _ in range(4)
            ],
            max_responses=4,
        )
        self.assertEqual(4, len(responses))
        self.assertEqual(4, instrumented.calls)
        self.assertEqual(4, instrumented.failures)
        self.assertTrue(all(response_text(item) == '{"steps":[' for item in responses))

    def test_model_artifact_write_is_append_only_and_checksummed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / "run.json"
            _write_new(path, "{}\n")
            self.assertEqual("{}\n", path.read_text(encoding="utf-8"))
            checksum = path.with_suffix(".sha256").read_text(encoding="utf-8")
            self.assertTrue(checksum.endswith("  run.json\n"))
            with self.assertRaises(PolicyError):
                _write_new(path, "{}\n")

    def test_local_descriptor_fails_closed_without_full_gpu_allocation(self) -> None:
        executor = BaseModelBenchmarkExecutor(
            system_id="base",
            provider=FakeBoundProvider(),
            source_snapshot_hash="b" * 64,
            runtime_collector=lambda _: runtime_environment("50% CPU/50% GPU"),
        )
        executor.execute(task())
        with self.assertRaises(PolicyError):
            executor.descriptor()

    def test_failed_capacity_check_preserves_runtime_observation(self) -> None:
        executor = BaseModelBenchmarkExecutor(
            system_id="base",
            provider=FakeBoundProvider(),
            source_snapshot_hash="b" * 64,
            runtime_collector=lambda _: RuntimeEnvironment(
                **{
                    **asdict(runtime_environment()),
                    "runtime_context_tokens": 4096,
                    "signature": "",
                }
            ),
        )
        observation = executor.execute(task())
        self.assertEqual("INSUFFICIENT_EVIDENCE", observation.terminal_state)
        self.assertIsNotNone(executor.provider.latest_runtime)
        with self.assertRaises(PolicyError) as context:
            executor.descriptor()
        self.assertIn("context is smaller", str(context.exception))

    def test_four_path_model_run_is_provider_and_hardware_bound(self) -> None:
        source_files = (SourceFileRecord(path="fixture", sha256="c" * 64),)
        source_hash = stable_hash([asdict(item) for item in source_files])
        executors = make_model_benchmark_executors(
            provider_factory=FakeBoundProvider,
            source_snapshot_hash=source_hash,
            runtime_collector=lambda _: runtime_environment(),
            runtime_releaser=lambda _: None,
        )
        run = run_benchmark(
            tasks=(task(),),
            executors=executors,
            generated_on="2026-08-28",
            package_version="test",
            git_head="head",
            worktree_dirty=True,
            source_files=source_files,
            execution_mode=MODEL_EXECUTION_MODE,
            qualification_status=MODEL_QUALIFICATION_STATUS,
        )
        self.assertEqual(BENCHMARK_SYSTEM_IDS, tuple(item["system_id"] for item in run.systems))
        self.assertFalse(run.performance_claim_allowed)
        self.assertIn(
            "super_reasoning_kernel_grounded_topology_v2_task_isolated:full_sr:",
            run.systems[2]["pipeline"],
        )
        self.assertEqual(12, run.systems[2]["telemetry"]["provider_calls"])
        self.assertEqual(10_000, run.results[2].correctness_bp)
        for descriptor in run.systems:
            self.assertTrue(descriptor["provider_binding"]["model_digest"])
            self.assertEqual("100% GPU", descriptor["runtime_environment"]["accelerator_allocation"])
            self.assertEqual(
                "single_run_not_reproducibility_evidence",
                descriptor["telemetry"]["nondeterminism_status"],
            )
        self.assertEqual(1, len(run.systems[2]["telemetry"]["certificate_signatures"]))

    def test_provider_bound_shards_merge_telemetry_and_task_order(self) -> None:
        source_files = (SourceFileRecord(path="fixture", sha256="c" * 64),)
        source_hash = stable_hash([asdict(item) for item in source_files])
        first_task = task()
        second_task = BenchmarkTask(
            schema_version=first_task.schema_version,
            problem_id="logic-test-2",
            category=first_task.category,
            prompt=first_task.prompt,
            oracle=first_task.oracle,
            oracle_method=first_task.oracle_method,
            required_evidence_ids=first_task.required_evidence_ids,
            required_counterexamples=first_task.required_counterexamples,
            source_refs=first_task.source_refs,
        )

        def shard(selected_task: BenchmarkTask):
            executors = make_model_benchmark_executors(
                provider_factory=FakeBoundProvider,
                source_snapshot_hash=source_hash,
                runtime_collector=lambda _: runtime_environment(),
                runtime_releaser=lambda _: None,
            )
            return run_benchmark(
                tasks=(selected_task,),
                executors=executors,
                generated_on="2026-08-29",
                package_version="test",
                git_head="head",
                worktree_dirty=True,
                source_files=source_files,
                execution_mode=MODEL_EXECUTION_MODE,
                qualification_status=MODEL_QUALIFICATION_STATUS,
            )

        first = shard(first_task)
        second = shard(second_task)
        merged = merge_benchmark_runs(
            shards=(second, first),
            tasks=(first_task, second_task),
        )
        self.assertEqual(2, merged.task_count)
        self.assertEqual(
            [first_task.problem_id, second_task.problem_id],
            [merged.results[0].problem_id, merged.results[3].problem_id],
        )
        for system_index, descriptor in enumerate(merged.systems):
            expected_calls = (
                first.systems[system_index]["telemetry"]["provider_calls"]
                + second.systems[system_index]["telemetry"]["provider_calls"]
            )
            self.assertEqual(expected_calls, descriptor["telemetry"]["provider_calls"])
            self.assertEqual(
                tuple(first.systems[system_index]["telemetry"]["response_hashes"])
                + tuple(second.systems[system_index]["telemetry"]["response_hashes"]),
                tuple(descriptor["telemetry"]["response_hashes"]),
            )
            self.assertEqual(
                first.systems[system_index]["telemetry"]["reasoning_validation_repairs"]
                + second.systems[system_index]["telemetry"]["reasoning_validation_repairs"],
                descriptor["telemetry"]["reasoning_validation_repairs"],
            )
        report = qualify_reasoning_runs(
            (merged, merged),
            ablation_runs={},
        )
        self.assertEqual(10_000, report.certificate_reproducibility_bp)
        with self.assertRaisesRegex(PolicyError, "assertion does not match"):
            qualify_reasoning_runs(
                (merged, merged),
                ablation_runs={},
                certificate_reproducibility_assertion_bp=0,
            )

    def test_held_out_provider_run_is_explicit_and_never_claim_authority(self) -> None:
        run = provider_bound_run()
        self.assertEqual(MODEL_HELD_OUT_QUALIFICATION_STATUS, run.qualification_status)
        self.assertFalse(run.performance_claim_allowed)

    def test_ablation_evidence_is_bound_to_persisted_pipeline(self) -> None:
        full = provider_bound_run(ablation_id="full_sr")
        one_path = provider_bound_run(ablation_id="one_path_only")
        report = qualify_reasoning_runs(
            (full, full),
            ablation_runs={
                "full_sr": (full,),
                "one_path_only": (one_path,),
            },
        )
        self.assertEqual(
            ("one_path_only", "full_sr"),
            report.present_ablation_ids,
        )
        self.assertEqual(2, len(report.ablation_evidence))
        with self.assertRaisesRegex(PolicyError, "pipeline mismatch"):
            qualify_reasoning_runs(
                (full, full),
                ablation_runs={"without_verifier": (one_path,)},
            )

    def test_repeated_qualification_rejects_provider_identity_drift(self) -> None:
        first = provider_bound_run(model_digest="a" * 64)
        second = provider_bound_run(model_digest="b" * 64)
        with self.assertRaisesRegex(PolicyError, "provider/runtime identity"):
            qualify_reasoning_runs(
                (first, second),
                ablation_runs={},
            )

    def test_provider_bound_run_releases_runtime_after_each_task(self) -> None:
        released = []
        source_files = (SourceFileRecord(path="fixture", sha256="c" * 64),)
        source_hash = stable_hash([asdict(item) for item in source_files])
        executors = make_model_benchmark_executors(
            provider_factory=FakeBoundProvider,
            source_snapshot_hash=source_hash,
            runtime_collector=lambda _: runtime_environment(),
            runtime_releaser=lambda profile: released.append(profile.model),
        )
        run_benchmark(
            tasks=(task(),),
            executors=executors,
            generated_on="2026-08-28",
            package_version="test",
            git_head="head",
            worktree_dirty=True,
            source_files=source_files,
            execution_mode=MODEL_EXECUTION_MODE,
            qualification_status=MODEL_QUALIFICATION_STATUS,
        )
        self.assertEqual(["fake-bound-model"] * 3, released)

    @mock.patch("ourd.reasoning.model_benchmark.time.sleep", return_value=None)
    @mock.patch(
        "ourd.reasoning.model_benchmark._ollama_allocation",
        side_effect=[("100% GPU", 4096), ("", 0)],
    )
    @mock.patch("ourd.reasoning.model_benchmark.urllib.request.urlopen")
    def test_ollama_release_uses_zero_keep_alive(
        self,
        urlopen,
        _allocation,
        _sleep,
    ) -> None:
        profile = bind_provider_profile(FakeBoundProvider())
        release_ollama_runtime(profile)
        request = urlopen.call_args.args[0]
        self.assertEqual("http://127.0.0.1:11434/api/generate", request.full_url)
        self.assertEqual(0, json.loads(request.data)["keep_alive"])


if __name__ == "__main__":
    unittest.main()
