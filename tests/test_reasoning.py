...
from ourd.providers.openai_responses import OpenAIResponsesProvider
...
from ourd.reasoning.generator import DEFAULT_PERSPECTIVES, parse_json_object, perspective_names
...
class ReasoningPersistenceAndIntegrationTests(unittest.TestCase):
    def test_runtime_v2_migrates_to_v4(self) -> None:
        fixture = RepoFixture()
        try:
            ...
            try:
                ...
                self.assertEqual({"from_schema": 2, "to_schema": 4}, events[0]["payload"]["migration"])
            finally:
                store.close()
        finally:
            fixture.close()
    ...
    def test_runtime_v3_migrates_pool_to_authoritative_hypothesis_state(self) -> None:
        ...
        try:
            ...
            payload["hypothesis_pool"] = payload.pop("reasoning_hypothesis_pool")
            payload.pop("reasoning_hypothesis_state", None)
            payload.pop("reasoning_hypothesis_updates", None)
            (state_dir / "state.json").write_text(json.dumps(payload), encoding="utf-8")
            store = StateStore(state_dir)
            try:
                migrated = store.load()
                ...
                self.assertEqual(reasoning_problem.problem_id, migrated.reasoning_hypothesis_state.problem_id)
                self.assertEqual(
    ...
    def test_multi_response_provider_preserves_order(self) -> None:
        provider = OpenAIResponsesProvider(
            ProviderConfig(
                ...
                max_reasoning_samples=2,
            )
        )
        with mock.patch.object(provider, "create_response", side_effect=["first", "second"]):
    ...
    def test_multi_response_provider_returns_one_error_per_request(self) -> None:
        ...
        with mock.patch.object(
            provider,
            "create_response",
            side_effect=[ProviderError("first failed"), "second"],
        ):
            responses = provider.create_responses(
                requests=[
                    {"instructions": "a", "input_items": [], "tools": []},
    ...
    def test_cfel_matching_falsifier_marks_hypothesis_falsified(self) -> None:
        ...
        self.assertEqual(0, state.reasoning_hypothesis_state.hypotheses[0].posterior_bp)
